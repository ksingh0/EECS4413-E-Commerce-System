	...
	public void createPayment(Payment payment) {
		String sql = """
				    INSERT INTO payments (
				        ...
				        card_holder_name, card_last_four_digits, card_type, card_token, expiry_date
				    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
				""";
		...
		try (Connection conn = DatabaseConnection.connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			pstmt.setString(13, payment.getCardLastFourDigits());
			pstmt.setString(14, payment.getCardType());
			pstmt.setString(15, payment.getCardToken());
			pstmt.setString(16, payment.getExpiryDate());
		...
		try (Connection conn = DatabaseConnection.connect();
			...
			while (rs.next()) {
				Payment payment = new Payment(rs.getInt("id"), rs.getInt("user_id"), rs.getInt("auction_id"),
						...
						rs.getString("card_holder_name"), rs.getString("card_last_four_digits"),
						rs.getString("card_type"), rs.getString("card_token"), rs.getString("expiry_date"));
